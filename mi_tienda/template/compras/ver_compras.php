...
$chartDataCompras = [];
$result = $conn->query("SELECT DATE(fecha) as fecha, COUNT(*) as total_compras FROM compras GROUP BY DATE(fecha) ORDER BY fecha ASC");
while ($row = $result->fetch_assoc()) {
    $chartDataCompras[] = $row;
}

// Obtener datos para el gráfico de gastos por día
$chartDataGastos = [];
$result = $conn->query("SELECT DATE(fecha) as fecha, SUM(total) as total_gastos FROM compras GROUP BY DATE(fecha) ORDER BY fecha ASC");
while ($row = $result->fetch_assoc()) {
    $chartDataGastos[] = $row;
}
?>
    ...
    <div class="container">
        ...
        <div class="collapse navbar-collapse" id="navbarNav">
            <ul class="navbar-nav ms-auto">
                <li class="nav-item">
                    <a class="nav-link btn btn-outline-primary" href="../compras.php">Volver a la Página Principal</a>
                </li>
            </ul>
        </div>
    </div>
...
<div class="text-center mt-4">
    ...
    <p class="text-muted">Consulta y administra las compras realizadas</p>
</div>
...
<div class="table-responsive mt-4">
    <table class="table table-hover table-bordered align-middle">
        ...
        <tbody>
            <?php
            $result = $conn->query("SELECT * FROM compras ORDER BY fecha DESC");
            while ($row = $result->fetch_assoc()) {
                echo "<tr>";
                echo "<td>" . $row["id"] . "</td>";
                ...
                echo "<td>";
                echo "<a href='actualizar_compra.php?id=" . $row["id"] . "' class='btn btn-warning btn-sm me-1'>";
...
<div class="row mt-5">
    ...
    <div class="col-md-6">
        <h3 class="text-center">💸 Gastos por Día</h3>
...
<script>
    ...
    // Datos para el gráfico de gastos por día
    const chartDataGastos = <?php echo json_encode($chartDataGastos); ?>;
    const labelsGastos = chartDataGastos.map(data => data.fecha); // Fechas
    const dataGastos = chartDataGastos.map(data => data.total_gastos); // Total de gastos

    // Configuración del gráfico de gastos por día
